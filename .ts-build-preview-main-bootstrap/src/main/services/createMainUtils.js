...
exports.createMainUtils = createMainUtils;
const electron_1 = require("electron");
const child_process_1 = require("child_process");
function createMainUtils({ axios, CONFIG, fs, exec, BrowserWindow, dialog }) {
    if (!axios)
        throw new Error('createMainUtils: axios is required');
    if (!CONFIG)
        throw new Error('createMainUtils: CONFIG is required');
    if (!fs)
        throw new Error('createMainUtils: fs is required');
    if (!exec)
        throw new Error('createMainUtils: exec is required');
    if (!BrowserWindow)
        throw new Error('createMainUtils: BrowserWindow is required');
    if (!dialog)
        throw new Error('createMainUtils: dialog is required');
    return {
        ...
        runCommand(command, options = {}) {
            ...
            return new Promise((resolve, reject) => {
                let stdoutAll = '';
                let stderrAll = '';
                const child = (0, child_process_1.spawn)(command, { shell: true, windowsHide: true });
        ...
        async checkAndInstallPrerequisites(mainWindow) {
            ...
            if (!pythonInstalled) {
                ...
                if (dialogResult.response === 0) {
                    electron_1.shell.openExternal('https://www.python.org/downloads/windows/');
                }
                return false;
            }
