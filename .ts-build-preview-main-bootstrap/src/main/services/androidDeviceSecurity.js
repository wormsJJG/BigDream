...
exports.createAndroidDeviceSecurityHelpers = createAndroidDeviceSecurityHelpers;
function createAndroidDeviceSecurityHelpers({ client, adbShell, adbShellWithTimeout }) {
    async function getEnabledAccessibilityPackages(serial) {
        try {
            ...
            console.warn('⚠️ 접근성 활성 서비스 목록 조회 실패:', error?.message || error);
            return new Set();
        }
    }
    async function getActiveDeviceAdminPackages(serial) {
        try {
            ...
            console.warn('⚠️ 기기 관리자 활성 목록 조회 실패:', error?.message || error);
            return new Set();
        }
    }
    ...
    async function setDeviceSecuritySetting(serial, settingId, enabled) {
        try {
            ...
            if (id === 'devOptions') {
                ...
                try {
                    await adbShellWithTimeout(target, `settings put secure development_settings_enabled ${on}`);
                }
                catch (_e) { }
            ...
            if (id === 'usbDebug') {
                try {
                    await adbShellWithTimeout(target, `settings put global adb_enabled ${on}`);
                }
                catch (_e) { }
                try {
                    await adbShellWithTimeout(target, `settings put secure adb_enabled ${on}`);
                }
                catch (_e) { }
                if (!enabled) {
                    try {
                        await adbShellWithTimeout(target, 'svc usb setFunctions mtp');
                    }
                    catch (_e) { }
                    try {
                        await adbShellWithTimeout(target, 'svc usb setFunctions none');
                    }
                    catch (_e) { }
                }
                return { ok: true, changed: true, settingId: id, enabled: !!enabled };
            ...
            if (id === 'wifiDebug') {
                try {
                    await adbShellWithTimeout(target, `settings put global adb_wifi_enabled ${on}`);
                }
                catch (_e) { }
                try {
                    await adbShellWithTimeout(target, `settings put secure adb_wifi_enabled ${on}`);
                }
                catch (_e) { }
                return { ok: true, changed: true, settingId: id, enabled: !!enabled };
            }
            if (id === 'securityAutoBlock') {
                ...
                for (const cmd of cmds) {
                    try {
                        await adbShellWithTimeout(target, cmd);
                    }
                    catch (_e) { }
                }
                return { ok: true, changed: true, settingId: id, enabled: !!enabled };
    ...
    async function openAndroidSettings(serial, screen) {
        try {
            ...
            else if (normalized === 'ACCESSIBILITY_SETTINGS')
                intent = 'android.settings.ACCESSIBILITY_SETTINGS';
            else if (normalized === 'DEVICE_ADMIN_SETTINGS')
                intent = 'android.settings.DEVICE_ADMIN_SETTINGS';
            else if (normalized === 'NOTIFICATION_LISTENER_SETTINGS')
                intent = 'android.settings.ACTION_NOTIFICATION_LISTENER_SETTINGS';
            else if (normalized === 'UNKNOWN_APP_SOURCES')
