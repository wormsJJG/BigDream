...
exports.createFirestoreService = createFirestoreService;
function createFirestoreService() {
    ...
    function decodeFieldValue(v) {
        if (v && typeof v === 'object' && v.__op === 'increment') {
            ...
            const encoded = v;
            const raw = (encoded.n !== undefined ? encoded.n : undefined) ??
    ...
    function decodeData(obj) {
        if (Array.isArray(obj))
            return obj.map(decodeData);
        if (!obj || typeof obj !== 'object')
            return decodeFieldValue(obj);
    ...
    function applyConstraints(baseQuery, constraints) {
        ...
        for (const c of (constraints || [])) {
            if (!c || !c.__type)
                continue;
            if (c.__type === 'where')
                clauses.push(where(c.field, c.op, c.value));
            if (c.__type === 'orderBy')
                clauses.push(orderBy(c.field, c.direction || 'asc'));
            if (c.__type === 'limit')
                clauses.push(limit(Number(c.n || 0)));
            if (c.__type === 'startAfter')
                clauses.push(startAfter(c.value));
        }
