...
type AppDetailManagerType = {
  lastScrollY: number;
  show(app: DetailApp, displayName: string): void;
  setupActionButton(btnId: string, text: string, app: DetailApp, appName: string): void;
};

export function initAppDetail(ctx: RendererContext): void {
  const setMainSubText = (
    el: HTMLElement | null,
    mainText: unknown,
    subText = '',
    subClass = 'bd-detail-sub'
  ) => {
    if (!el) return;
    el.replaceChildren();
  ...
  const AppDetailManager: AppDetailManagerType = {
    lastScrollY: 0,

    show(app, displayName) {
      console.log('상세 정보 표시 실행:', displayName, '유형:', app.isApkFile ? 'APK' : '설치됨');

      const iconWrapper = document.querySelector('.detail-icon-wrapper') as HTMLElement | null;
      ...
      const dashboard = document.getElementById('results-dashboard-view') as HTMLElement | null;
      const detailView = document.getElementById('app-detail-view') as HTMLElement | null;
      const resultsHeader = document.querySelector('.results-header') as HTMLElement | null;
      const privacyNotice = document.getElementById('privacy-footer-notice') as HTMLElement | null;
      const scrollContainer = document.querySelector('#logged-in-view .main-content') as HTMLElement | null;
      ...
      if (appNameEl) appNameEl.textContent = app.cachedTitle || displayName;
      if (packageEl) packageEl.textContent = app.packageName || '';
      ...
      const neutralizeBtnEl = document.getElementById('neutralize-btn') as HTMLButtonElement | null;
      const uninstallBtnEl = document.getElementById('uninstall-btn') as HTMLButtonElement | null;

      const detailItems = Array.from(document.querySelectorAll('#app-detail-view .detail-item'));
      const bgLabel = detailItems?.[1]?.querySelector('.d-label') as HTMLElement | null;
      const netLabel = detailItems?.[2]?.querySelector('.d-label') as HTMLElement | null;
      ...
      if (app.isApkFile) {
        ...
        if (reqEl) reqEl.textContent = String((app.requestedList || app.permissions || []).length);
        ...
        if (networkEl) {
          ...
          setMainSubText(
            networkEl,
            `총 ${Utils.formatBytes(total)}`,
            `(수신: ${Utils.formatBytes(usage.rx)} / 송신: ${Utils.formatBytes(usage.tx)})`,
            'bd-detail-sub bd-detail-sub--sm'
          );
        }
        ...
        const reqEl = document.getElementById('detail-req-count');
        const grantEl = document.getElementById('detail-grant-count');
        if (reqEl) reqEl.textContent = String(app.requestedCount || 0);
        if (grantEl) grantEl.textContent = String(app.grantedCount || 0);
      }
      ...
      if (reqCountEl) reqCountEl.textContent = String(totalCount);
      if (grantCountEl) {
        ...
        if (perms.length > 0) {
          ...
          if (app.isApkFile) {
            Utils.renderPermissionCategoriesReadOnly(perms, list, {
              mode: 'apk',
              getLabel: (p: string) => Utils.getKoreanPermission(p),
            });
          } else {
            Utils.renderPermissionCategoriesReadOnly(perms, list, {
              mode: 'installed',
