...
type ClientFieldKey = 'name' | 'dob' | 'phone';

type ClientInputMap = Record<ClientFieldKey, HTMLInputElement | null>;
type AnonCheckMap = Record<ClientFieldKey, HTMLInputElement | null>;
type AnonValueMap = Record<ClientFieldKey, string>;

export function initClientDevice(ctx: RendererContext): void {
  const { State, ViewManager, CustomUI, dom } = ctx;
  const { loggedInView } = dom;

  const clientInfoForm = document.getElementById('client-info-form') as HTMLFormElement | null;
  const toConnectionScreenBtn = document.getElementById('to-connection-screen-btn') as HTMLButtonElement | null;
  const clientInputs: ClientInputMap = {
    name: document.getElementById('client-name') as HTMLInputElement | null,
    dob: document.getElementById('client-dob') as HTMLInputElement | null,
    phone: document.getElementById('client-phone') as HTMLInputElement | null
  };

  const anonChecks: AnonCheckMap = {
    name: document.getElementById('anon-name') as HTMLInputElement | null,
    dob: document.getElementById('anon-dob') as HTMLInputElement | null,
    phone: document.getElementById('anon-phone') as HTMLInputElement | null
  };

  const anonValues: AnonValueMap = {
    name: '익명 사용자',
    dob: '0001-01-01',
    phone: '000-0000-0000'
  };

  function setupAnonToggle(key: ClientFieldKey): void {
  ...
  if (disconnectBtn) {
    disconnectBtn.addEventListener('click', async () => {
      ...
      if (await CustomUI.confirm(confirmMsg)) {
        ...
        let resDevOff: any = null;

        if (!isLoaded && isAndroid) {
          const api = (window.electronAPI || {}) as any;
          const doAction = async (action: unknown) => {
            try {
          ...
          let devNow: boolean | null = null;
          const toBool = (status: unknown): boolean | null => {
          ...
          const readSecurityStatus = async () => {
            ...
            const devItem = items.find((it: any) => it && it.id === 'devOptions');
        ...
        const navScanInfo = document.getElementById('nav-scan-info') as HTMLElement | null;
        if (navScanInfo) {
        ...
        const iosSubMenu = document.getElementById('ios-sub-menu') as HTMLElement | null;
        if (iosSubMenu) {
        ...
        const dashNav = document.getElementById('nav-android-dashboard') as HTMLElement | null;
        if (dashNav) {
        ...
        try { ctx.helpers?.stopAndroidDashboardPolling?.(); } catch (_) { /* noop */ }
        try { ctx.helpers?.resetAndroidDashboardUI?.(); } catch (_) { /* noop */ }
        try { ctx.helpers?.setDashboardScrollLock?.(false); } catch (_) { /* noop */ }
        ...
        const realStartScanBtn = document.getElementById('real-start-scan-btn') as HTMLButtonElement | null;
        if (realStartScanBtn) {
  ...
  const DeviceManager = {
    ...
    async checkDevice() {
      ...
      try {
        const android = await window.electronAPI.checkDeviceConnection() as any;
      ...
      try {
        const ios = await window.electronAPI.checkIosConnection() as any;
    ...
    setUI(status: string, titleText: string, descText: string, color: string, showBtn = true) {
      ...
      const btnContainer = document.getElementById('start-scan-container') as HTMLElement | null;

      if (title) {
        title.textContent = titleText;
        (title as HTMLElement).style.color = color;
      }
      if (desc) {
      ...
      if (status === 'connected') {
        ...
        if (alertTitle) alertTitle.innerHTML = 'DEVICE<br>READY';
        ...
        if (alertTitle) alertTitle.innerHTML = 'WAITING<br>AUTH';
      } else {
        wrapper?.classList.add('state-disconnected');
        if (alertTitle) alertTitle.innerHTML = 'CONNECT<br>DEVICE';
      }
    }
  };
