export function createResultsRenderer(ctx, deps) {
    ...
    const ResultsRenderer = {
        render(data) {
            ...
            const inferDeviceMode = (payload) => {
                ...
                if (payload?.mvtResults || payload?.mvtAnalysis || payload?.mvt)
                    return 'ios';
                if (typeof payload?.runningCount === 'number')
                    return 'android';
                if (Array.isArray(payload?.apkFiles) && payload.apkFiles.length > 0)
                    return 'android';
                // 4) fallback
            ...
            if (data?.deviceInfo && !data.deviceInfo.os)
                data.deviceInfo.os = detectedMode;
            ...
            if (isIos && Array.isArray(State.scanRuntime?.androidListCleanup)) {
                State.scanRuntime.androidListCleanup.forEach(fn => { try {
                    fn && fn();
                }
                catch (_) { } });
            ...
            try {
                ...
                if (modeEl)
                    modeEl.textContent = isIos ? 'MVT기반 분석 + BD_SFA 행동 분석' : 'ADB + BD_SFA 행동 분석';
                ...
                if (narrationEl) {
                    ...
                    if (!isIos)
                        parts.push(`백그라운드 실행 ${runningCount}개`);
                    if (!isIos && apkCount > 0)
                        parts.push(`발견된 APK ${apkCount}개`);
                ...
                if (stepsEl) {
                    ...
                    if (isIos) {
                        ...
                        if (apkCount > 0)
                            steps.push(`저장소에서 발견된 APK 파일 ${apkCount}개를 추가 수집해 설치 대기/유입 경로 위험을 평가합니다.`);
            ...
            try {
                ...
                if (isIos) {
                    ...
                    if (threatsTitle)
                        threatsTitle.textContent = "🔍 상세 분석 결과 (5대 핵심 영역)";
                    if (threatsDesc)
                        threatsDesc.textContent = "스파이웨어 흔적 탐지를 위한 5가지 시스템 영역 분석 결과입니다.";
                    // 2. 검사 대상 앱 목록 설명 추가 및 제목 업데이트
                    const totalApps = data.allApps ? data.allApps.length : 0;
                    if (appsHeader)
                        appsHeader.textContent = `📲 검사 대상 애플리케이션 목록 (총 ${totalApps}개)`;
                    if (iosAppDesc) {
                    ...
                    const iosPrivacyApps = buildIosPrivacyThreatApps(normalizedApps, Array.isArray(data.privacyThreatApps) ? data.privacyThreatApps : []);
                    try {
                    ...
                    // 1. 안드로이드 원래 문구로 복구 
                    if (threatsTitle)
                        threatsTitle.textContent = "🔐 기기 보안 상태";
                    if (threatsDesc)
                        threatsDesc.textContent = "스파이앱 침입 가능성을 높이는 설정을 점검합니다.";
                    if (iosAppDesc)
                        iosAppDesc.style.display = 'none'; // 안드로이드에선 숨김
