export function initIosAppListControls({ State, Utils, apps, container }) {
...
export function renderSuspiciousList({ suspiciousApps, isIos, formatAppName }) {
...
export function renderPrivacyThreatList({ privacyApps, clear, formatAppName }) {
    const containers = [
    ...
    const normalizedApps = (Array.isArray(privacyApps) ? privacyApps : [])
        ...
        const identifier = String(app.packageName
            || app.bundleId
            || app.identifier
            || app.id
            || `unknown.app.${index + 1}`);
        const displayName = String(app.cachedTitle
            || app.name
            || app.displayName
            || app.title
    ...
    const buildChips = (items, host) => {
        if (!Array.isArray(items) || items.length === 0 || !host)
            return;
        items.forEach((x) => {
    ...
    const buildReasons = (reasons, host) => {
