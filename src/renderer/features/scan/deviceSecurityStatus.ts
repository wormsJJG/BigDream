    ...
    try {
      if (!container.__dsBound) {
        container.addEventListener('click', async (ev) => {
          ...
          const btn = target && target.closest ? target.closest('.ds-action-btn') as HTMLButtonElement | null : null;
          if (!btn) return;
          ...
          if (!actionPayload || !actionPayload.kind) return;
          ...
          try {
            ...
            try { btn.disabled = false; } catch (_e) { /* noop */ }
            try { btn.textContent = oldText; } catch (_e) { /* noop */ }
          }
        });
