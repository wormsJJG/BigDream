...
import { getNormalizedScanApps } from '../features/scan/scanInfo.js';
        ...
        export const Utils = {
            ...
            categorizePermission(permString) {
            ...
            makePermChip(p, updateSelectAll) {
            const chip = document.createElement('button');
            chip.type = 'button';
            chip.className = 'bd-perm-chip';
            chip.dataset.perm = p;
            chip.dataset.selected = '0';
            chip.textContent = this.getKoreanPermission(p);
            ...
            for (const p of perms) {
                const cat = this.categorizePermission(p);
            ...
            for (const cat of cats) {
                ...
                const isDefaultOpen = this.DEFAULT_OPEN_CATS.has(cat);
                ...
                for (const p of list) {
                body.appendChild(this.makePermChip(p, updateSelectAll));
                }
            ...
            // 권한 분석 상세용
            renderPermissionCategoriesReadOnly(perms, container, options = {}) {
            const {
                grantedSet = new Set(),
                mode = 'installed',
                getLabel = (p) => this.getKoreanPermission(p),
            } = options;
            ...
            for (const cat of cats) {
                ...
                if (mode === 'apk') {
                    el.className = 'perm-item perm-apk';
                    el.textContent = "🔍 " + getLabel(p);
                } else {
                    const isGranted = grantedSet.has(p);
                    el.className = `perm-item ${isGranted ? 'perm-granted' : 'perm-denied'}`;
                    el.textContent = (isGranted ? '✅ ' : '🚫 ') + getLabel(p);
                }
