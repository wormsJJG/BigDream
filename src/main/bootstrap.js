...
function start({ rootDir }) {
  ...
  // -----------------------------------------------------------------------------
  // Read text files inside the app directory (used for HTML partial/template loading)
  // -----------------------------------------------------------------------------
//   ipcMain.handle('read-text-file', async (event, payload) => {
//     try {
//       const relativePath = String(payload?.relativePath || '');

//       // Only allow reading files within the app root directory.
//       // Block path traversal like ../..
//       // app.getAppPath() points to the application root (or the asar root in production).
//       // This is the most reliable base for reading bundled templates.
...
//     } catch (err) {
//       console.error('[read-text-file] failed:', err);
//       throw err;
//     }
//   });
  ...
  // ------------------------------
  // Firebase (client SDK) in main
  // ------------------------------
  let __firebaseApp = null;
  let __firebaseAuth = null;
  let __firestoreDb = null;

  function __ensureFirebase() {
      if (__firebaseApp && __firebaseAuth && __firestoreDb) return;

      const { initializeApp, getApps } = require('firebase/app');
      const { getAuth } = require('firebase/auth');
      const { getFirestore } = require('firebase/firestore');

      // IMPORTANT: This is the same config as renderer firebaseConfig.js
      ...
      if (getApps().length === 0) {
          __firebaseApp = initializeApp(firebaseConfig);
      } else {
          __firebaseApp = getApps()[0];
      }

      __firebaseAuth = getAuth(__firebaseApp);
      __firestoreDb = getFirestore(__firebaseApp);
  }

  // Main-auth login/logout so Firestore rules work (renderer logs in, then tells main to login too)
  ipcMain.handle('firebase-auth-login', async (_evt, { email, password }) => {
      __ensureFirebase();
      const { signInWithEmailAndPassword } = require('firebase/auth');
      if (!email || !password) throw new Error('email/password required');
      const cred = await signInWithEmailAndPassword(__firebaseAuth, email, password);
  ...
  ipcMain.handle('firebase-auth-logout', async () => {
      ...
      await signOut(__firebaseAuth);
      return { ok: true };
  });

  function __decodeFieldValue(v) {
      // Support increment / serverTimestamp from renderer proxy encoding
      if (v && typeof v === 'object' && v.__op === 'increment') {
          const { increment } = require('firebase/firestore');
          return increment(Number(v.n || 0));
      }
  ...
  function __decodeData(obj) {
      if (Array.isArray(obj)) return obj.map(__decodeData);
      if (!obj || typeof obj !== 'object') return __decodeFieldValue(obj);
      const out = {};
      for (const [k, v] of Object.entries(obj)) out[k] = __decodeData(v);
      return out;
  }

  function __docRef(pathSegments) {
      const { doc } = require('firebase/firestore');
      return doc(__firestoreDb, ...pathSegments);
  }

  function __colRef(pathSegments) {
      const { collection } = require('firebase/firestore');
      return collection(__firestoreDb, ...pathSegments);
  }

  function __applyConstraints(baseQuery, constraints) {
      const {
          where,
          orderBy,
          limit,
      ...
      return q(baseQuery, ...clauses);
  }

  // Firestore CRUD proxy entrypoint
  ipcMain.handle('firestore-call', async (_evt, payload) => {
      __ensureFirebase();

      // If rules require auth, renderer should call firebase-auth-login first.
      // We keep this check explicit so errors are clearer than "permission-denied".
      if (!__firebaseAuth.currentUser) {
          throw new Error('NOT_AUTHENTICATED_IN_MAIN');
      }
      ...
      switch (op) {
          case 'getDoc': {
              const ref = __docRef(pathSegments);
              const snap = await getDoc(ref);
              return { exists: snap.exists(), id: snap.id, data: snap.exists() ? snap.data() : null };
          }
          case 'setDoc': {
              const ref = __docRef(pathSegments);
              await setDoc(ref, __decodeData(payload.data || {}), payload.options || {});
              return { ok: true };
          }
          case 'updateDoc': {
              const ref = __docRef(pathSegments);
              await updateDoc(ref, __decodeData(payload.data || {}));
              return { ok: true };
          }
          case 'deleteDoc': {
              const ref = __docRef(pathSegments);
          ...
          case 'addDoc': {
              const ref = __colRef(pathSegments);
              const added = await addDoc(ref, __decodeData(payload.data || {}));
              return { ok: true, id: added.id };
          }
          case 'query': {
              const base = __colRef(pathSegments);
              const qq = __applyConstraints(base, payload.constraints || []);
