...
export function createIosMvtExecutionHelpers({
    ...
    function createMvtStatsRefresher(outputDir: string) {
        ...
        return async function refreshMvtStats(force = false): Promise<DirectoryStats> {
            const now = Date.now();
            ...
            try {
                const stat = await new Promise<DirectoryStats>((resolve) => {
                    setImmediate(() => {
                        try {
    ...
    async function maybeExtendMvtStage({
        onProgress,
        progressPolicy,
        backupElapsedSec,
        mvtStageStartedAt,
