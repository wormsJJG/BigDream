type AdbClientLike = {
    shell(serial: string, cmd: string): Promise<unknown>;
    pull(serial: string, remotePath: string): Promise<NodeJS.ReadableStream & {
...
type AdbLike = {
    util: {
        readAll(stream: unknown): Promise<Buffer>;
    };
};
...
function buildFallbackTitle(packageName: string) {
...
async function loadGooglePlayScraper() {
    ...
    return (mod.default || mod) as {
        app(options: { appId: string; lang?: string; country?: string }): Promise<{ title?: string; icon?: string }>;
    };
}
...
export function createAndroidAppInventoryHelpers({
    client,
    adb,
    ApkReader,
    fs,
    ...
    async function enrichInstalledAppsWithStoreMetadata(apps: Array<Record<string, unknown>>) {
        const storeApps = apps.filter((app) =>
            app &&
        ...
        for (let i = 0; i < storeApps.length; i += 6) {
            ...
            chunk.forEach((app, index) => {
                ...
                if (!preview) {
                    if (!app.cachedTitle) app.cachedTitle = buildFallbackTitle(String(app.packageName || ''));
                    return;
                }
    ...
    async function getInstalledApps(serial: string) {
    ...
    async function getAppInstallTime(serial: string, packageName: string) {
        try {
    ...
    async function checkIsRunningBackground(serial: string, packageName: string) {
        try {
    ...
    async function getAppPermissions(serial: string, packageName: string) {
        try {
            const output = await client.shell(serial, `dumpsys package ${packageName}`);
            const dumpsys = (await adb.util.readAll(output)).toString();
    ...
    async function getNetworkUsageMap(serial: string) {
        const usageMap: Record<string, { rx: number; tx: number }> = {};
        try {
            let data = '';
            try {
            ...
            data.split('\n').forEach((line) => {
                ...
                if (trimmedLine.startsWith('ident=')) {
                    ...
                    if (uidMatch) {
                        ...
                        if (!usageMap[currentUid]) usageMap[currentUid] = { rx: 0, tx: 0 };
                    } else {
                        currentUid = null;
                    }
    ...
    async function getApkPermissionsOnly(serial: string, remotePath: string) {
        let tempPath: string | null = null;
        try {
            tempPath = path.join(os.tmpdir(), `extract_${Date.now()}.apk`);
            const transfer = await client.pull(serial, remotePath);
            await new Promise<void>((resolve, reject) => {
                const fileWriter = fs.createWriteStream(tempPath as string);
                transfer.on('end', () => {
                    if (typeof (fileWriter as { end?: () => void }).end === 'function') {
                        (fileWriter as { end: () => void }).end();
                    }
                });
            ...
            const permissions: string[] & { __packageName?: string | null } = (manifest.usesPermissions || []).map((permission) => permission.name);
    ...
    async function findApkFiles(serial: string) {
        ...
        const allApkData: Array<Record<string, unknown>> = [];
        ...
        for (const searchPath of searchPaths) {
            try {
                ...
                for (const line of lines) {
                    ...
                    let apkManifestPackage: string | null = null;
                    try {
                        const perms = await getApkPermissionsOnly(serial, filePath);
                        apkManifestPackage = perms && (perms as typeof perms & { __packageName?: string | null }).__packageName
                            ? (perms as typeof perms & { __packageName?: string | null }).__packageName || null
                            : null;
                    } catch (_e) { }
                ...
                console.error(`${searchPath} 검색 실패:`, (error as Error).message);
            }
        }
