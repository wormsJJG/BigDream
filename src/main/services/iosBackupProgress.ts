...
export function createIosBackupProgressHelpers({
    fs,
    path,
    Utils,
    formatBytes,
    ...
    async function getUsedBytes(infoPath: string, udid: string) {
        const keyTry = async (key: string) => {
            try {
    ...
    function buildBackupMessage(bytes: number, files: number) {
        return `아이폰 백업 진행 중... (파일 ${files.toLocaleString('en-US')}개 / ${formatBytes(bytes)})`;
    }

    function hasMeaningfulBackupStarted(bytes = 0, files = 0, trustedCount: TrustedCount = null) {
    ...
    function moveToward(current: number, target: number) {
    ...
    function createBackupStatsRefresher(specificBackupPath: string) {
        const statsCache: RefreshStatsResult & { scannedAt: number; running: boolean } = {
            bytes: 0,
            files: 0,
            scannedAt: 0,
            running: false
        ...
        return async function refreshBackupStats(force = false): Promise<RefreshStatsResult> {
            const now = Date.now();
            ...
            try {
                const stat = await new Promise<RefreshStatsResult>((resolve) => {
                    setImmediate(() => {
                        try {
    ...
    function computeTheaterTargetPct(elapsedSec: number, targetSec: number) {
