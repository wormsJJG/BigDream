...
exports.createIosBackupCacheHelpers = createIosBackupCacheHelpers;
function createIosBackupCacheHelpers({ fs, path, getDirectoryStats, removeDirectorySafe, safeEmit, trustPromptMessage }) {
    ...
    function emitReuseReady(onProgress) {
        safeEmit(onProgress, {
            step: 1,
            totalSteps: 2,
            stage: 'device-check',
    ...
    async function resolveReusableBackup({ specificBackupPath, backupMetaPath, onProgress }) {
